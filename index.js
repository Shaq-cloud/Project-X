...
app.post('/api/v1/devices/register', asyncHandler(async (req, res) => {
  ...
  if (!hardwareUuid || !deviceName || !customerName || !customerEmail || !phoneNumber || !totalContractAmount || !paymentFrequency || !paymentAmount) {
  ...
  const deviceAddress = req.body.deviceAddress || normalizedAddress;
  ...
  if (!acceptedFrequencies.has(paymentFrequency)) {
    const err = new Error('Invalid payment frequency');
    err.status = 400;
    throw err;
  }

  const totalAmount = Number(totalContractAmount);
  const paymentAmountValue = Number(paymentAmount);
  if (Number.isNaN(totalAmount) || totalAmount <= 0 || Number.isNaN(paymentAmountValue) || paymentAmountValue <= 0) {
    ...
    try {
      ...
      switch (payload.event) {
        case 'charge.success': {
          ...
          const amount = formatPaystackAmount(amountKobo || 0);
          const metadata = data.metadata || {};
          const contractId = metadata.contract_id;

          if (!contractId) {
            logger.warn('Missing contract_id in webhook metadata; skipping processing');
            break;
          }
          ...
          try {
            ...
            if (newPaid >= total) {
              ...
              const releaseToken = generateReleaseToken(hardwareUUID, tokenSecret);
              ...
              const unlockToken = generateTimeToken(hardwareUUID, tokenPeriod.year, tokenPeriod.month, tokenSecret);
